...
const examplePredicatesComment = `// Here go the type predicates
// generated for the types on the left.

`;

const generatedByComment = `// The source code below is automatically generated by the type-predicate-generator.
// https://peter-leonov.github.io/type-predicate-generator/
//
// It is not generally meant to be modified by hand, but
// if you still feel like changing it you can do so.
// The future versions of the generator are going to keep
// generating readable code that is easy to merge your changes
// into.

`;

const exampleTestsComment = `// And below you can find the unit tests
...
function onChange() {
  ...
  try {
    ...
    const example = sourceCode === defaultExample;
    ...
    if (predicatesCode) {
      predicatesEditor.setValue(
        (example ? examplePredicatesComment : generatedByComment) +
          predicatesCode
      );
    }
    if (testsCode) {
      ...
      testsEditor.setValue(
        (example ? exampleTestsComment : generatedByComment) +
          testsCode
      );
    } else {
      testsEditor.setValue("");
